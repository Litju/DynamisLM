...
SERIALIZATION_EFFECT=No threshold value is embedded in a registered method and no project-wide default is introduced. The current version 3 payload normalizes finite integer detector values to their float representation; affected canonical event payloads and hash-derived event identifiers must be deliberately re-materialized. Prior v2 envelopes are rejected by the strict decoder; no old non-positive value is silently migrated or reinterpreted.
