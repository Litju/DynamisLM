...
def test_promotion_is_one_conjunctive_authority_and_cannot_be_forged() -> None:
    decision = decide_promotion(
        runtime_integrity_pass=True,
        source_qualified=True,
        population_qualified=True,
        raw_bytes_verified=True,
        source_version_verified=True,
        license_captured=True,
        variable_identity_resolved=True,
        football_world_mapping_resolved=True,
    ...
    failed = decide_promotion(
        ...
        source_version_verified=True,
        license_captured=True,
        variable_identity_resolved=False,
        football_world_mapping_resolved=True,
        lineage_complete=True,
    )
    assert not failed.can_promote
    assert failed.status is PromotionStatus.QUARANTINED
    with pytest.raises(ValueError):
        replace(failed, can_promote=True)
...
def test_canonical_replay_is_independent_of_input_order_and_keeps_lineage() -> None:
    ...
    assert canonical_replay_digest((first, second)) == canonical_replay_digest((second, first))
    validate_canonical_replay((first, second), (second, first))
